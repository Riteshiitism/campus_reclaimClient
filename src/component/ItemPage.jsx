  ...
  useEffect(() => {
    Axios.get(`${API}/item/` + item_id)
      .then((response) => {
        ...
        // console.log(data);
        const created_date = new Date(data.createdAt);
        data.createdAt = `${created_date.getDate()}/${created_date.getMonth()}/${created_date.getFullYear()} ${created_date.getHours()}:${created_date.getMinutes()}`;
        setItem(data);
        setArr(data.image);
  ...
  useEffect(() => {
    Axios.get(`${API}/answers/` + item_id).then((response) => {
      if (response.data.length > 0) {
        ...
        // console.log(data);
        const created_date = new Date(data.createdAt);
        const t = `${created_date.getDate()}/${created_date.getMonth()}/${created_date.getFullYear()} ${created_date.getHours()}:${created_date.getMinutes()}`;
        setD(t);
